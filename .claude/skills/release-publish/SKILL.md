...
CI：构建 Windows + macOS 安装包 → 上传到 GitHub Release（草稿）
...
本项目只构建 **Windows + macOS**（不构建 Linux），减少 CI 时间和产物体积。
...
| **自动更新兜底 + 备份存档** | GitHub 公开 `bkywksj/knowledge-base-release` | **备源** | R2 不通时兜底 + 海外备份 |

> Gitee 已跳过（token 曾过期，当前无需）。若未来要加 Gitee 三级分发，参考 tauri-cc 的 skill。
...
| **更新端点（R2 主）** | `https://pub-9d9e6c0cb6934fb0a0c505e3c64f39b2.r2.dev/knowledge-base/update.json` |
| **更新端点（GitHub raw 备）** | `https://github.com/bkywksj/knowledge-base-release/raw/main/update.json` |
...
| **源码仓库分支** | `master`（GitHub remote 名为 `github`）|
| **Release 仓库（GitHub 公开）** | `https://github.com/bkywksj/knowledge-base-release` |
| **本地 Release 仓库路径** | `E:/my/桌面软件tauri/knowledge-base-release` |
| **Release 仓库分支** | `main`（GitHub 新建仓库默认，**不是 master**）|
...
**要下载的 9 个文件**（CI 产物前缀 `Knowledge.Base_`，注意文件名带点）：
...
> ⚠️ **dmg 带版本号**，**app.tar.gz 不带版本号**，这是 tauri-action 的约定。
> ⚠️ 用户容易漏下 `.exe` 主文件（浏览器只下 `.sig`），清点时要核对**所有 9 个都在**。
...
### 步骤 10：推送 release 仓库（产物 + update.json + update-r2.json）
