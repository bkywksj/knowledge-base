...
///
/// 1500 这个值由编辑器工具栏定：它有 48 个按钮、总宽约 1720 逻辑像素，而每行可用宽
/// ≈ 窗口宽 − 570（左侧导航栏 + 笔记面板 + 大纲面板 + 工具栏内边距）。1500 → 每行 ~930，
/// 两行容量 1860 装得下 1720，工具栏稳定 2 行；1408 时每行 ~840，会掉到 3 行。
pub const DEFAULT_WIDTH: f64 = 1500.0;
...
mod tests {
    ...
    fn t_1080p_uses_percentage() {
        // 1920×1080：宽 75% = 1440 撞 floor 1500（工具栏 2 行的下限），高 88% = 950 未撞
        assert_eq!(size(1920.0, 1080.0), (1500, 950));
    }
    ...
    fn t_small_screen_falls_back_to_floor_then_clamps() {
        // 1366×768 老本：宽 75%=1024 撞 floor 1500，再被 95%=1297.7 夹回来；
    ...
    fn t_2k_scaled_configs() {
        assert_eq!(size(1707.0, 960.0), (1500, 845)); // 27" 2K @150%：宽走 floor
