...
> 创建日期：2026-04-26
>
> **2026-05-23 复盘**：Q-001/Q-002/Q-003/Q-004 全部修复（详见各条目）。
> Q-001 (5b6ee21) / Q-002 (4ad86af) / Q-003 (8930076) / Q-004 (dd214ff)。
> 仅剩 Q-strategy-1（公告 + B 站置顶，非代码任务）。
...
- **状态**：✅ `completed` · 已修复（commit `4ad86af`，2026-04-26）—— list_notes 加 `include_descendants` 字段（默认 true），点父文件夹递归显示所有子孙笔记，子文件夹笔记加 ↳ 视觉标记。"单击一会有一会没有"实际是用户在切换不同子文件夹时的视觉错觉，递归修复后归因消失。
...
- **状态**：✅ `completed` · 首次打开弹引导 notification，告知"已加入本地库 + 编辑写回原文件"（2026-05-23）
- **来源**：安分 04-26 11:01
