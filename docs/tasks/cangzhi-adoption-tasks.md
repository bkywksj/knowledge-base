...
| ~~v61~~ | ~~P1-5~~ | 收件箱：`inbox_items` —— **已连同功能整体回滚**（见 P1-5），版本号回收 |
| **v61** | P2-1 | 切片层：`note_chunks` | ⬜ |
| **v62** | P2-2 | 向量层：`embedding_profiles` / `chunk_embeddings` | ⬜ |
| **v63** | P2-4 | 任务队列：`bg_jobs` | ⬜ |
...
| 任务 | commit | 一句话 |
|---|---|---|
| P1-1 中文加权 n-gram 召回 | 1dd5600 | 修「词在中间搜不到」（反向验证过：改造前返回 `[]`） |
| P1-2 搜索筛选维度 | fc48202 | 文件夹/标签/时间/类型，下推 SQL + fail-closed |
| P1-3a Excel 解析增强 | b7045e8 | **合并单元格回填**（藏知零处理）+ CSV/TSV |
| P1-3b Excel 数据集 | a3443d2 + 736e01e | schema v60 + 字段画像 + UI，**真机验证** |
| P1-4 搜索结果定位 | c332865 | **核实后确认已存在，无需实施** |
| ~~P1-5 收件箱~~ | ee9925a…de740a0 → **已回滚** | 做完才想明白性价比不够，整体删除（理由见 P1-5 章节） |

**两次「动手前核实」省下的白工**：
- P1-4 整条不用做（基线 agent 只看搜索页没追到编辑器侧的误判）
- P1-5 的三个失败点评估后明确不接（收益低 / 语义不对），而不是硬凑完整度

**两次真机验证抓到的**：schema v59 在真实数据上加密了 2 条 Key；
收件箱「忽略」触发三处联动（条目移除 / 徽章递减 / 筛选器自动隐藏）。
...
- Excel 预览拆成「原样预览 / 数据集」两个视图（前者保留原有行为）
- **列头即画像**：类型徽章 + 语义角色（Tooltip 说明，其中 identifier 特意写明
  "不适合求和" —— 这是画像里最有实际价值的一条）+ 完整度（仅不足 100% 时显示）
  + 去重取值数
- 同 sheet 多区域时标「区域 N」；无表头标橙色 Tag
- 「重新识别」按钮走 `force=true`（平时靠哈希跳过重复解析）
- 顺带把 **csv/tsv 改走表格预览** —— P1-3a 后端已能正确处理引号内逗号与 GBK，
  但前端仍当纯文本渲染成一堆逗号
- 行 JSON 解析抽到 `src/lib/datasetRows.ts` + 5 个测试
  （项目惯例只测 lib 纯函数；"单行坏数据不该让整页空掉"值得钉住）

**✅ 真机验证通过**（`pnpm tauri dev`，2026-08-21）

用一个含「说明行 + 主表 + 小计表」的 sheet + 一个「合并表头」sheet 的样本，
在真实应用里逐项确认：

| 验证点 | 结果 |
|---|---|
| schema v59 迁移（P0-1a） | 日志 `已加密 2 条 Key` —— **在真实数据上生效** |
| schema v60 迁移 | `当前版本: 60`，三张表建好 |
| 区域切分 | 「识别出 3 个数据集」，与样本完全吻合 |
| 同 sheet 多区域 | Tab 显示「销售明细 区域1 / 区域2」 |
| **合并单元格回填** | `2024年度` 出现在 A/B/C 三列（Excel 里只有 A1 有值） |
| 无表头降级 | 橙色「无表头」Tag + 列名回退 `A列/B列/C列` |
| **标识优先于度量** | `订单编号` 判为「标识」而非「度量」 |
| 类型推断 | `销售金额`→数字/度量，`下单日期`→日期/时间 |
| 完整度仅不足时显示 | `客户名称` 显示橙色 75%（有一行为空），其余列不显示 |
| CASCADE | 删数据集后 `dataset_rows` 孤儿行为 0 |

**顺带修的**：翻页时会重复拉一次 schema（列画像不随页码变）。已拆成两个
effect（1a71cdf）。

**验证时的工具坑（非应用问题）**：
- Tauri 用 rfd 文件对话框，agileshot 的 `handle_file_dialog` 点「打开」被它当成取消
  → `EditorToolbar.tsx` 的 `if (!selected) return;` 静默返回，附件插不进去。
  改为直接构造附件文件 + 注入链接绕过。
- `screenshot_window` 用 PrintWindow 抓窗口自身内容，**被别的窗口遮挡时照样能看见**，
  但点击打不到。曾有个误触发的文件对话框盖在前面，导致连续几次点击"无反应"，
  排查了好几轮才发现。**以后点击前先确认目标窗口在前台**。
...
### P1-5　失败可重试的收件箱　❌ 已实现又整体删除

> **结论先行**：做完了、真机验证过，然后评估下来**性价比不够，整体回滚**。
> schema v61 从未随任何版本发布（最后一个 tag v1.52.0 是 2026-08-05，
> 这批提交在 8-21），所以直接删掉迁移、把 v61 号让给 P2-1，**不写降级迁移**。

**为什么删（三条，按分量排）**

1. **核心卖点「一键重试」在唯一大规模接入的 PDF 上是假的。**
   `pdf.rs:598-628` 里 PDF 只有三种失败原因（知网中文字体编码 / xref 损坏 / 已加密），
   给出的修复建议全是"用 Chrome 打印另存为 PDF"或"先解密"——
   **修完是另一个文件、另一条路径**，收件箱记的 `source` 指向老文件，点重试照样失败。
   剩下那条 OCR 路径也站不住：能走到收件箱的恰恰是"OCR 已经试过还失败"的。
2. **半接状态比没有更糟。** `InboxKind` 声明了 6 种、收件箱页也写了 6 个重试分支，
   实际只有 `import_pdf` 和 `clip` 有生产者；Word 的失败弹窗自始至终没接
   （`showImportFailModal` 的注释却写着「PDF / Word 共用」）。
   用户会问"为啥 PDF 失败记下来了、Word 就没了"，这种说不清的不一致最伤体验。
3. **它换来的只是「跨重启的失败备忘」，却要付一个新概念的钱** ——
   侧栏一格 + 一个红点 + 一张表 + 5 个 Command + 一套重试分派。
   而这是个知识库应用，"记东西"本来就有地方。

**替代方案（已随删除一起落地，成本约 15 行）**
- 批量导入失败弹窗加「复制清单（含完整路径与原因）」—— 这个弹窗现在是唯一记录，
  复制出去贴进笔记 / 待办慢慢处理，比在应用里再养一个队列实在
- 剪藏失败**不关弹窗**，URL 还在输入框里，再点一次就是重试（单条动作不需要队列）

---

<details>
<summary>以下为删除前的原始设计记录（保留备查）</summary>
...
- **Word 导入**：失败结构与 PDF 本就一样，把 `showPdfFailModal` 泛化成
  `showImportFailModal(fail, title, kind, folderId)` 两处共用 ——
  否则"落库 + 提示"要维护两遍，加第三种格式还会再抄一遍
- **网页剪藏**：原先只有 `message.error` 一闪就没。剪藏失败常是临时性的
  （限流 / 需登录 / 网络抖动），过一阵重试往往就成
- **重试分派补齐**：`import_word` → `importWordFiles`，`clip` → `noteApi.clipUrl`

**❌ 评估后明确不接的三个（不是遗漏）**

| 失败点 | 不接的理由 |
|---|---|
| Markdown 导入 | `errors` 是格式化好的字符串，要接得先改 `ImportResult` 结构 + 5 处 `errors.push`。而 md 失败率远低于 PDF（只是编码/IO，没有扫描件这类问题），且部分错误本就不是文件级的（"创建根文件夹失败"/"清理空日期文件夹失败"），硬塞进"某个文件失败了"的语义不对 |
| OCR 独立入口 | 设置页的 OCR 是用户主动测试，失败当场就知道；PDF 内的 OCR 失败已被 `import_pdf` 覆盖 |
| 数据集识别 | 失败当场显示在面板里，且文件还在附件中随时可重试 |

`InboxKind` 保留完整枚举（含 `import_md` / `ocr` / `dataset`）—— 那是后端 schema
注释里就定义好的**数据字典**，不是为不存在的需求造的抽象。
...
## 2026-08-21 实测：用数据决定 P2 做不做

动手前先量了一轮真实语料（dev 库 225 篇，其中 **RAG 可见仅 40 篇** ——
184 篇已删除、1 篇隐藏）。方法：临时 `#[ignore]` 探针跑真实 `search_notes_for_rag`，
量完即删（不进提交）。

### 结论 1：召回不是瓶颈 → 向量层（P2-2）暂无必要

8 条「语义相关但用词不同」的查询，**8/8 全部命中**，5 条排第 1：

| 查询 | 期望 | 最佳名次 |
|---|---|---|
| 怎么把接口定义自动变成代码 | OpenAPI 代码生成插件 | #1 |
| 知识库的 MCP 工具 | kb-mcp | #1 |
| 绩效考核为什么容易走偏 | …误区（走偏≠误区） | #1 |
| 第一次使用 Claude 该怎么开始 | 装完就用·从 0 到第一次对话 | #1 |
| 后端权限管理怎么做的 | ruoyi 后端框架（标题无"权限"二字） | #1 |
| 本地 git 仓库怎么配置 | 本地仓库说明 | #3 |
| 相亲软件的功能规划 | 陌生交友项目 | #5 |
| 移动端 App 用什么框架做的 | plus-uniapp 移动端框架 | #10 |

P1-1 加权 n-gram + P1-2 筛选 + P0-2 RRF 已经够用。**省下 7-10 天。**

⚠️ 有效性边界：40 篇的语料上成立。语料涨到几千篇、或实测出真实 miss，再重启此项。

### 结论 2：开窗才是瓶颈 → 切片层的痛点用 1/20 成本解决了

`extract_window_for_rag` 旧实现只围绕**最早**一处命中开一个 16000 字窗：

| 笔记 | 查询 | 命中总数 | 旧覆盖 | 新覆盖 |
|---|---|---|---|---|
| id=142（48981 字，AGENTS.md） | 配置 | 46 | 15% | **87%** |
| id=142 | 错误 | 23 | 22% | **87%** |
| id=142 | 接口 | 17 | 35% | **82%** |
| id=135（281875 字） | 错误 | 117 | 10% | 41% |
| id=135 | 配置 | 165 | 15% | 35% |
| id=145（19161 字） | 全部 | — | 97~100% | 97~100%（**不变**） |

模型常只看到"文档开头顺带提一句"的地方，真正展开讲的段落全被切掉 ——
用户体感就是"AI 答得零碎"。改成**命中聚簇 + 贪心选至多 4 段**（8f80186）。

**不回退闸门**：单窗口已覆盖 ≥80% 命中时原样返回单窗口，
命中本就集中的笔记（id=145）行为一字不变。

id=135 提升有限是物理限制：16000 字预算对 28 万字只占 5.7%，
再怎么选段也盖不全 —— 那种超长笔记该靠**拆分笔记本身**解决，不是检索的事。

### 结论 3：一条计划里的「成本为零」改进其实收益也为零

原计划写着「索引字段扩成 标题 + heading_path + 正文（藏知 `processor.py:1182-1190`，
成本为零）」。**核实后不成立**：藏知那条成立是因为它是**切片级**索引，
切片只含正文段落、heading 在切片外面，不加就丢了。
我们是**文档级 FTS**（`notes_fts(title, fts_body)`），heading 本来就在 content 里、
早被索引了。抄计划时没验的一条。
...
### P2-1　切片层（chunking）　⏸️ 降级：核心痛点已用 1/20 成本解决（8f80186）

> **不是不做，是先做了便宜那 90%。** 见下方「2026-08-21 实测」。
> 真正剩下的场景（长文档局部主题被整篇低分淹没 → 影响**排序**）暂未观测到，
> 等有实例再立项。

**原现状描述**：我们**完全没有切片**（`chunk` 在 Rust 侧只命中 SQLite 变量分批、HTTP 流解码）。
长笔记整篇参与召回，查询时才用 `extract_window_for_rag`（`services/ai.rs`）截窗 ——
长文档的局部主题会被整篇的低分淹没。
...
### P2-2　向量检索（含 canary 兼容护栏）　⏸️ 实测判定：暂无必要

> **2026-08-21 实测结论：召回不是瓶颈，先不做。** 依据见下方实测数据。
> 设计（含 canary 护栏）保留在下面，等语料规模上来或实测出 miss 再启动。

> 🔴 **铁律（启动时仍适用）：canary 指纹护栏必须和向量同批上线，不能后补。**
